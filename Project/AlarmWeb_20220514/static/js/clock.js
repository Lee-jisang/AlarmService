...
setInterval(function(){
	...
	if (alarmTime == currentTime) { //설정한 알람시간이 현재시간과 같으면
		sound.play(); //알람 재생
		}
...
function alarmClear() {
    //여기서 웹캠화면페이지로 넘어가서 눈을 몇번 깜빡이면 알림 중지
	document.getElementById('alarmhrs').disabled = false;   //시간
	document.getElementById('alarmmins').disabled = false;  //분
	document.getElementById('alarmsecs').disabled = false;  //초
