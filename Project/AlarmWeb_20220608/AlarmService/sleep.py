...
class Blink_Detector(object):
    ...
    def eyeBlinkDetection(self):
        global check
        if self.check_blink == True and self.pred_l > 0.5 and self.pred_r > 0.5:    # 눈동자가 감겼으면서 양쪽 눈동자를 뜬경우
        ...
        if self.eye_count_min > 10:
            print(self.eye_count_min)
            check = True
            time.sleep(1)
            return True
    ...
    def get_frame(self):
        ...
        for face in faces:
            ...
            cv2.putText(self.image, state_l, tuple(eye_rect_l[0:2]), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
            cv2.putText(self.image, state_r, tuple(eye_rect_r[0:2]), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
        ...
        # 1분을 초로 출력
        cv2.putText(self.image, state_min, (0, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
        # 1분동안 눈동자 깜빡임 횟수 출력
        cv2.putText(self.image, state_count, (0, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
